---
title: "示例 6.2: 使用Python/ASE和GAP的硅搜索"
layout: single
classes: wide
lang: zh
...
需要QUIP（带有GAP扩展）以及quippy。完整的安装说明在[这里](https://github.com/libAtoms/QUIP)，但通过pip安装quippy应该就足够了：

```console
$ pip install quippy-ase
```

还需要GAP数据文件，即可从[此处](https://www.repository.cam.ac.uk/handle/1810/317974)获得的Si_PRX_GAP势。将`Si_PRX_GAP`目录放在此README所在的目录中：
...
此搜索确实产生了预期的立方金刚石基态（空间群`Fd-3m`），但仅一次。通常，可能需要进行更大的搜索来找到它。

---

**上一页：** [示例 6.1: 使用Python/ASE的Lennard-Jones固体]({{ '/zh/tutorials/6.1/' | relative_url }})
